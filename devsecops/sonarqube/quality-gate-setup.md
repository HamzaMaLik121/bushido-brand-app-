...
> **Location:** `devsecops/sonarqube/quality-gate-setup.md`  
> **Pipeline reference:** `devsecops/sonarqube/quality-gate-setup.md`
...
- The "New Code" period defines which code changes are evaluated against the quality gate.
- Recommended setting: **30 days** — only code changed in the last 30 days is assessed.
- Configure this in SonarQube under **Administration** → **Configuration** → **New Code Period**.

## Quality Gate in Jenkins

The Jenkins pipeline uses `waitForQualityGate abortPipeline: true` to block the build if the quality gate fails. This requires a SonarQube webhook pointing to Jenkins:
